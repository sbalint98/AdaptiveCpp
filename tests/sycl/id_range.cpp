...
void test_id_range_operators() {
...

  {
    ...
    if(dimensions >= 1) BOOST_TEST_REQUIRE(result[0] == 8);
    if(dimensions >= 2) BOOST_TEST_REQUIRE(result[1] == 11);
    if(dimensions == 3) BOOST_TEST_REQUIRE(result[2] == 20);
  }

  {
    // T + size_t
    const auto result = test_value + 2;
    if(dimensions >= 1) BOOST_TEST_REQUIRE(result[0] == 7);
    if(dimensions >= 2) BOOST_TEST_REQUIRE(result[1] == 9);
    if(dimensions == 3) BOOST_TEST_REQUIRE(result[2] == 13);
  }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(range_api, _dimensions, test_dimensions::type) {
  ...
  }
  {
    // Equality operator
    s::range<d> range;
    BOOST_TEST_REQUIRE(!(range == test_value));
    range = test_value;
    BOOST_TEST_REQUIRE((range == test_value));
  }
  {
...

  {
    ...
    if(d >= 1) BOOST_TEST_REQUIRE(range.get(0) == 5);
    if(d >= 2) BOOST_TEST_REQUIRE(range.get(1) == 7);
    if(d == 3) BOOST_TEST_REQUIRE(range.get(2) == 11);
  }
  {
    // range::operator[]
  ...
  }
  {
    ...
    const auto range = test_value;
    if(d >= 1) BOOST_TEST_REQUIRE(range[0] == 5);
    if(d >= 2) BOOST_TEST_REQUIRE(range[1] == 7);
    if(d == 3) BOOST_TEST_REQUIRE(range[2] == 11);
  }
  {
    // range::size()
    const auto range = test_value;
    BOOST_TEST_REQUIRE(range.size() == 5 * (d >= 2 ? 7 : 1) * (d == 3 ? 11 : 1));
  }
...
BOOST_AUTO_TEST_CASE_TEMPLATE(id_api, _dimensions, test_dimensions::type) {
  ...
  }
  {
    // Equality operator
    s::id<d> id;
    BOOST_TEST_REQUIRE(!(id == test_value));
    id = test_value;
    BOOST_TEST_REQUIRE((id == test_value));
  }
  {
  ...
  }
  {
    ...
    if(d >= 1) BOOST_TEST_REQUIRE(id.get(0) == 5);
    if(d >= 2) BOOST_TEST_REQUIRE(id.get(1) == 7);
    if(d == 3) BOOST_TEST_REQUIRE(id.get(2) == 11);
  }
  {
    // id::operator[]
    auto id = test_value;
  ...
  }
  {
    // const id::operator[]
    const auto id = test_value;
    if(d >= 1) BOOST_TEST_REQUIRE(id[0] == 5);
    if(d >= 2) BOOST_TEST_REQUIRE(id[1] == 7);
    if(d == 3) BOOST_TEST_REQUIRE(id[2] == 11);
  }
